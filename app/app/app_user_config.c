...
static AppUserConfig s_conf_default = {
    ...
    .NumKeyLightTime= 0,
    .Language       = APP_LANG_POLISH,   /* 与旧版 Polish 默认一致 */
...
{
    printf("[UserConfig] reset to default\n");
    AppLanguage lang = s_conf.Language;
    s_conf = s_conf_default;
    s_conf.Language = lang;   /* 保留语言设置，与旧版 UserConfigReset 一致 */
    AppUserConfigSave();
    return 1;
}
...
int AppUserConfigInit(void)
{
    ...
    if (fd < 0) {
        printf("[UserConfig] %s not found, using default\n", USER_CONFIG_PATH);
        s_conf = s_conf_default;
        AppUserConfigSave();
        return 0;
    }
    ...
    printf("[UserConfig] loaded: lang=%d unlock=%ds ungate=%ds voice=%d\n",
