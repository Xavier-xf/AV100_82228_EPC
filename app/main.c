...
int main(int argc, char *argv[])
{
    ...
    LOG_I("\n###############################################");
    LOG_I("# Door Entry System  Build: %s %s", __DATE__, __TIME__);
    LOG_I("\n###############################################");
     ...
     * 注意：DrvAdcInit / DrvKeypadInit / DrvCardInit 等
     *       产生回调的驱动，在 App 注册回调之后再启动。
     * ================================================ */
    // INIT_MODULE(DrvWdtOpen(WDT_TIMEOUT_SEC));    /* 硬件看门狗（10s）*/

    INIT_MODULE(DrvGpioInit());         /* GPIO 引脚初始化（LED/继电器/功放）*/
    INIT_MODULE(DrvAudioInInit());      /* 音频输入（启动采集线程，默认未采集）*/
    INIT_MODULE(DrvVideoInInit());      /* 视频输入（第三路 SVP 回调在 SvcSvpInit 后注册）*/
    ...
    /* 对讲音视频流（协议号由 SvcIntercomStreamStart 时按 peer_dev_id 动态计算）*/
    INIT_MODULE(SvcIntercomStreamInit());

    /* ================================================
     * Step 6: App 层（先注册回调和事件订阅）
     * ================================================ */
    ...
    LOG_I("\n###############################################");
    LOG_I("# System Ready. Device=DOOR%d", door_id);
    LOG_I("\n###############################################");
